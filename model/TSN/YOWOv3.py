...
class YOWOv3(torch.nn.Module):
    def __init__(self, num_classes, backbone2D, backbone3D, interchannels, mode, img_size, pretrain_path=None,
        ...
        dummy_img3D = torch.zeros(1, 3, 5, img_size[0], img_size[1])
        dummy_img2D = torch.zeros(1, 3, img_size[0], img_size[1])
    ...
    def load_pretrain(self, pretrain_yowov3):
        ...
        if isinstance(pretrain_state_dict, dict) and "model" in pretrain_state_dict:  # new trained model is py dict that contains 'model' key value but pretrained model does not
            epoch = pretrain_state_dict["epoch"]
