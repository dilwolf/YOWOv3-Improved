...
class DFLHead(torch.nn.Module):
    ...
    def __init__(self, nc, img_size, interchannels, filters, mode='decoupled'):
        ...
        # Pass actual input size
        if isinstance(img_size, (list, tuple)):
            self.img_size = img_size  # keep the pair
        else:
    ...
    def forward(self, x):
        ...
        if self.training:
            return x
        self.anchors, self.strides = (x.transpose(0, 1) for x in util.make_anchors(x, self.stride, 0.5))
    ...
    def initialize_biases(self):
        ...
        for a, b, s in zip(m.box, m.cls, m.stride):
            ...
            H, W = self.img_size
            area = (H / s) * (W / s)
            b[-1].bias.data[:m.nc] = math.log(5 / m.nc / area)